...
public class InfoProvider extends ContentProvider {
    ...
    /*이 부분은 MainActivity의 getContentResolver().query(uri,columns,null,null,"name ASC");에
    * 해당하는 부분이다. 인자로 넘어온 name ASC는 이름을 기준으로 오름차순 정렬을 의미한다.*/
    @Nullable
    @Override
    public Cursor query(Uri uri, String[] strings, String s, String[] strings1, String s1) {
        ...
        switch (uriMatcher.match(uri)) {
            case INFOS:
                ...
                * 마지막은 정렬 기준을 적으면 됨
                * (현재 우리 입장에서 정렬 기준은 name ASC로 이름을 기준으로 오름차순 작업을 진행한다.) */
    ...
    /*MainActivity의 getContentResolver().insert(uri,values)은 아래 코드에 해당한다.*/
    @Nullable
    @Override
    public Uri insert(Uri uri, ContentValues contentValues) {
        /*첫 번째 인자는 테이블 이름
        * 두 번째 인자는 null,
        * 세 번째 인자는 contentValues 로
        * 실제 칼럼과 각 컬럼에 들어갈 데이터에 해당한다.*/
    ...
    public int delete(Uri uri, String s, String[] strings) {
        ...
        * 무조건 */
        switch (uriMatcher.match(uri)) {
            case INFOS:
                /*실제 DB를 delete 하는 작업이 여기서 진행된다. 그리고 단순히 첫 번째 인자가
                * TABLE_NAME, 두 번째(name) 인자는 무엇을 기반으로 지울 것인지
                * 세번째(Jessica) 인자는 무엇이 되는 대상
    ...
    /*MainActivity에 있는 getContentResolver().update(url,updateValue,selection,selectionArgs)
    * 은 여기 잇는 update에 해당한다.*/
    @Override
    public int update(Uri uri, ContentValues contentValues, String s, String[] strings) {
        ...
        switch (uriMatcher.match(uri)) {
            case INFOS:
                ...
                * 폰 번호를 010-3377-2473으로 바꾸려 하는 것
                * 세 번째 인자는 무엇(mobile)을 update 하려고 하는가
                * 네 번째 세 번째 인자의 세부 사항
                * 즉 mobile 인데 010-3333-7777인 것을 변경하겠다는 의미*/
